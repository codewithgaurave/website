...
import { Landmark, Building2, Building, Castle, AlignEndHorizontal } from 'lucide-react';
import HotelStaffHiringModal from '@/components/modals/HotelStaffHiringModal';

// India Map Vector Icon
function IndiaSilhouetteIcon() {
  return (
    <svg 
      viewBox="0 0 1024 1024"
      preserveAspectRatio="xMidYMid meet"
      className="w-7 h-7 sm:w-8 sm:h-8 fill-[#024a9d] transition-transform duration-500 group-hover:scale-110"
    >
      <g transform="translate(0,1024) scale(0.100000,-0.100000)">
...
export default function Presence() {
  ...
  const cities = [
    ...
    { name: "Kolkata", icon: AlignEndHorizontal },
  ...
  return (
    <>
      <section className="py-16 sm:py-24 bg-slate-50 relative overflow-hidden border-t border-slate-100/50">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 w-full">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-y-10 sm:gap-y-12 gap-x-4 sm:gap-x-6">
            ...
            {/* 16th Item: Pan India Cities with India Map + Badge */}
            <div 
              onClick={() => setIsModalOpen(true)}
              className="flex flex-col items-center justify-center group cursor-pointer"
            >
              {/* Card with India Map Silhouette & Red Plus Badge */}
              <div className="w-14 h-14 sm:w-16 sm:h-16 rounded-2xl bg-white shadow-[0_5px_15px_-5px_rgba(0,0,0,0.05)] border border-slate-100 flex items-center justify-center mb-3 sm:mb-4 relative group-hover:-translate-y-2 group-hover:shadow-[0_15px_30px_-5px_rgba(2,74,157,0.15)] group-hover:border-blue-100 transition-all duration-300">
                ...
                {/* Red Circular Badge with Plus */}
                <span className="absolute top-2 right-2 sm:top-2.5 sm:right-2.5 w-3.5 h-3.5 sm:w-4 sm:h-4 bg-[#d62423] text-white rounded-full flex items-center justify-center font-black text-[9px] sm:text-[10px] shadow-sm ring-1.5 ring-white z-20">
                  +
                </span>
              </div>

              {/* +250 Pan India Cities text */}
              <div className="flex flex-col items-center">
                <span className="text-sm sm:text-base font-black text-[#d62423] tracking-tight leading-none mb-1">
                  +250
                </span>
                <span className="text-[10px] sm:text-[11px] font-bold text-slate-600 group-hover:text-slate-900 transition-colors leading-tight text-center whitespace-nowrap">
                  Pan India Cities
                </span>
              </div>
            </div>
      ...
      {/* Booking Modal on City / Pan India click */}
