...
import DailyBasisBookingForm from '@/components/forms/DailyBasisBookingForm';

export default function DailyCookPage() {
  return (
    <main className="min-h-screen bg-white font-sans text-center selection:bg-blue-600 selection:text-white">
      ...
      <section className="pt-[130px] sm:pt-[140px] md:pt-[150px] pb-12 px-4 sm:px-6 lg:px-8 max-w-[1240px] mx-auto relative text-center">
        <div className="absolute top-28 left-1/2 w-[600px] h-[350px] rounded-full bg-red-50/60 blur-[100px] -z-10 -translate-x-1/2"></div>

        {/* Top Badge */}
        <div className="bg-white border border-slate-200 text-[#024a9d] text-[11px] sm:text-[12px] font-black uppercase tracking-widest px-5 py-2.5 rounded-full mb-4 shadow-sm inline-flex items-center relative mt-2">
          <span className="w-2 h-2 rounded-full bg-[#d62423] mr-2"></span>
          FLEXIBLE DAILY BASIS CHEF BOOKING
        </div>
        
        <h1 className="text-[34px] sm:text-[46px] font-black text-[#0f172a] leading-[1.15] mb-4 tracking-tight max-w-4xl mx-auto">
          Hire Cook & Chef for Daily Basis / Events
        </h1>
        
        <p className="text-[#64748b] text-[15px] sm:text-[17px] font-medium leading-relaxed mb-6 max-w-2xl mx-auto">
          Book verified chefs and kitchen helpers for birthdays, house parties, family functions, or urgent restaurant relief.
        </p>

        {/* Daily Basis Booking Form directly at Top */}
        <div className="mt-4 mb-8">
          <DailyBasisBookingForm />
        </div>

        {/* Features Stack/Grid */}
        <div className="flex flex-col sm:flex-row flex-wrap justify-center gap-3 sm:gap-4 mb-5 max-w-4xl mx-auto text-left sm:text-center pt-6">
          {[
            { icon: '👨‍🍳', text: '12,500+ Staff Placed' },
            { icon: '🎉', text: '10,000+ Events Catered' },
            { icon: '🇮🇳', text: 'PAN India Hiring' },
            { icon: '⚡', text: 'Instant Shift Booking' }
          ].map((feat, i) => (
             <div key={i} className="bg-white border border-slate-200 shadow-sm px-5 py-3 rounded-xl flex items-center justify-center gap-3 text-[14px] font-bold text-[#0f172a] w-full sm:w-auto hover:border-[#024a9d] transition-colors relative group">
